...
function detailClear(){
	$('#detailFixDate').textbox('clear');
	$('#detailaccSellName').textbox('clear');
	$('#detailaccSellSum').textbox('clear');
	$('#detailaccSellRemark').textbox('clear');
	document.getElementById("detailaccSellFlag").checked=false
}
...
function newCarFixInfo(){
	...
	detailClear();
	//删除id = -1（临时）的配件信息
	$.post('/dfapi?api=125',{id:"-1"},function(result){
		if (result.success){
...
function destroyCarFixInfo(){
	var row = $('#dg_car').datagrid('getSelected');
	if (row){
		$.messager.confirm('Confirm','你确定要删除这条提货记录吗？',function(r){
			if (r){
				$.post('/dfapi?api=114',{id:row.id},function(result){
					if (result.success){
						$('#dg_car').datagrid('reload');	// reload the user data
					} else {
						$.messager.show({	// show error message
...
function fixAddAcc(){
	var accIDNumber = $('#accName').combobox('getValue');
	var accSellID = $('#id').val();
	$.post('/dfapi?api=122',{accIDNumber:accIDNumber, accSellID:accSellID},function(result){
		if (result.errorMsg){
			$.messager.show({
				title: '错误',
				msg: result.errorMsg
...
//		$('#fmidnumber').textbox('readonly', true);
		url_acc = '/dfapi?api=123&oldAccID='+row.carAccIDNumber;
